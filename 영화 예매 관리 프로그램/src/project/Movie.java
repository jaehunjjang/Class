...
import java.util.ArrayList;		//자바 API불러오기
...
public class Movie {			
	...
	public Movie (long id, String title, String genere) { //생성자
		this.id = id;
		this.title = title;
		this.genre = genre;
	}
	...
	public static void delete(String movieIdStr) throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(file));
		String text = "";								//파일을 복사하기 위한 빈 문자열
		String line = null;
		
		while((line = br.readLine()) != null) {
												//파일을 행 단위로 읽어옴(반복)
			String[] temp = line.split(",");	//쉽표 기준으로 문자열을 나눔
